...
$$\frac{\partial \rho}{\partial t} + \rho \frac{\partial \rho}{\partial x} = \frac{1}{Re} \frac{\partial^2 \rho}{\partial x^2}$$

#### **2. Proper Orthogonal Decomposition (POD)**
The state $\rho(x,t)$ is approximated by a low-rank projection:
$$\rho(x,t) \approx \bar{\rho} + \Phi \hat{\rho}(t)$$
Where $\Phi \in \mathbb{R}^{N \times K}$ contains the $K$ dominant modes.

#### **3. Discrete Empirical Interpolation Method (DEIM)**
To avoid the $O(N)$ bottleneck, we use **DEIM** to interpolate the non-linear flux $F(\rho)$:
$$\hat{F}(\rho) \approx \Psi (P^T \Psi)^{-1} P^T F(\rho)$$
